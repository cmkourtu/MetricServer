...
const createSubQueryArrayLiteral = (Model, queryOptions) => {
    ...
    return literal(
        `( SELECT DISTINCT("InnerSubQuery".id) FROM (${createSubQuery(Model, {
            ...
            limit: undefined,
            offset: undefined,
        })}) as "InnerSubQuery" LIMIT ${queryOptions.limit} OFFSET ${queryOptions.offset})`
    );
};
 ...
 */
const findAndCountAllPaginated = (Model, queryOptions, pageIndex, perPageCount) => {
    ...
    return Promise.all([
        ...
        sequelize
            .query(
                `SELECT DISTINCT("InnerSubQuery".id), "InnerSubQuery".rnum
                 FROM (${createSubQuery(Model, {
                     ...queryOptions,
                     attributes: [
                         "id",
                         literal(
                             `row_number() OVER (ORDER BY ${
                                 queryOptions.order ? getOrderQuery(queryOptions.order) : "id DESC"
                             }) as rnum`
                         ),
                     ],
                     distinct: true,
                 ...
                 ORDER BY "InnerSubQuery".rnum LIMIT ${perPageCount}
